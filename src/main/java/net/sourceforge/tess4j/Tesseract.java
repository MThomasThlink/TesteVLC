...
public class Tesseract {
    ...
    public static synchronized Tesseract getInstance() {
        if (instance == null) {
            System.out.println("Criando nova instancia de Tesseract!!\n\n");
    ...
    public String doOCR(List<IIOImage> imageList, Rectangle rect) throws TesseractException {
        ...
        for (IIOImage oimage : imageList) {
            ...
            try {
                ...
                System.err.printf("doOCR(List<IIOImage>) ERROR: %s. \n", ioe.getMessage());
            }
        }
    ...
    public String doOCR(int xsize, int ysize, ByteBuffer buf, Rectangle rect, int bpp) throws TesseractException {
        ...
        try {
            if (getInstance() == null)
                ...
                System.out.printf("Usando instância que já existia.");
            ...
            //prop.setProperty("classify_bln_numeric_mode", "1");
            //prop.setProperty("tessedit_char_whitelist", "0123456789");
            ...
            String utf8Text = hocr ? api.TessBaseAPIGetHOCRText(handle, pageNum - 1) : api.TessBaseAPIGetUTF8Text(handle);
            System.out.println("api.TessBaseAPIClear()");
            api.TessBaseAPIClear(handle);
            api.TessBaseAPIDelete(handle);
            return new String(utf8Text.getBytes(), "utf8");
            
        /*} catch (UnsupportedEncodingException uee) {
            System.out.printf("doOCR ERROR: %s.\n", uee.toString());
